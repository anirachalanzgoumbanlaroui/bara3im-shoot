...
from apps.daily_sessions.models import DailyEmployeePerformance, WorkDay, SellerDailyOperation
...
class StatisticsService:
    ...
    def get_statistics(employee):
        ...
        if role == 'seller':
            return StatisticsService._get_seller_statistics(employee, today)

        return StatisticsService._get_photographer_statistics(employee, today, role)

    @staticmethod
    def _get_seller_statistics(employee, today):
        ...
        for i in range(7):
            ...
            daily_photo_counts.append({
                ...
                'photos': int(op_d.amount) if op_d else 0,
                'date': d.isoformat(),
            })

        all_time_earnings = float(SellerDailyOperation.objects.filter(
            seller=employee
        ).aggregate(total=Sum('amount'))['total'] or 0.0)
        best_day = float(SellerDailyOperation.objects.filter(
            seller=employee
    ...
    @staticmethod
    def _get_photographer_statistics(employee, today, role):
        # 1. Monthly Overview
        start_of_month = date(today.year, today.month, 1)
        if today.month == 12:
        ...
        for p in month_perf:
            u_price = float(p.work_day.calculate_employee_unit_price(role, p.photo_count))
